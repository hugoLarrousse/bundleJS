...
function processType(type) {
  api.searchProductsByType(type).then(function(arr) {
    updateTable('similarTable', arr);
...
function processPrice(price) {
  api.searchProductsByPrice(price, 50).then(function(arr) {
    updateTable('similarTable', arr);
  }).catch(function(errorMessage){
    alert(errorMessage);
  })
...
document.getElementById("inputButtonSearch").addEventListener('click',function(){
  processSearch(document.getElementById('inputSearch').value);
});
